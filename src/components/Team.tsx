...
function Team({ profile }: TeamProps) {
  ...
  const isInList = DUMMY_BROKERS.some(
    (b) => b.name.toLowerCase() === profileName.toLowerCase()
  )
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {brokers.map((broker, index) => {
                ...
                const isCurrentUser =
                  profileName &&
                  broker.name.toLowerCase() === profileName.toLowerCase()
                ...
                return (
                  <tr
                    key={broker.name}
                    className={
                      highlighted ? 'bg-primary-50' : 'hover:bg-gray-50'
                    }
                  >
        ...
        <div className="space-y-3">
          {DUMMY_ACTIVITY.map((item, index) => {
            const isYou =
              item.actor === 'You' ||
              (profileName &&
                item.actor.toLowerCase() === profileName.toLowerCase())
